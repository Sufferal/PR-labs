...
def create_car():
  if not crud.leader and ("Token" not in request.headers or request.headers["Token"] != "Leader"):
    return jsonify({"message": "Only the leader can create"}), 403
...
def update_car(car_id):
  if not crud.leader and ("Token" not in request.headers or request.headers["Token"] != "Leader"):
    return jsonify({"message": "Only the leader can update"}), 403
...
def delete_car(car_id):
  if not crud.leader and ("Token" not in request.headers or request.headers["Token"] != "Leader"):
    return jsonify({"message": "Only the leader can delete"}), 403
